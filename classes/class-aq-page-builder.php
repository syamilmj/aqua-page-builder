...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['page_slug'] = 'aqua-page-builder';
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to resolve this issue', 'aqpb') .'</p></div>';"));
			}
		...
		**/
		function update_template($template_id, $blocks, $title) {
			...
			if(!$this->is_template($template_id)) wp_die( __('Error : Template id is not valid', 'aqpb') );
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . __( 'For more information:', 'aqpb' ) . '</strong></p>' .
					'<p>' . sprintf( __( '%s Documentation %s', 'aqpb' ), '<a href="http://aquagraphite.com/api/documentation/aqua-page-builder" target="_blank">', '</a>') . '</p>' .
					'<p>' . sprintf( __( '%s Changelog %s', 'aqpb' ), '<a href="https://wordpress.org/plugins/aqua-page-builder/changelog/" target="_blank">', '</a>' ) . '</p>'
				);
				
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> __( 'Overview', 'aqpb' ),
