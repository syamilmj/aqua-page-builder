...
if ( ! current_user_can( 'edit_theme_options' ) )
	wp_die( __( 'Cheatin&#8217; uh?', 'aq-page-builder' ) );
...
$template_name = isset( $_REQUEST['template-name'] ) && !empty( $_REQUEST['template-name'] ) ? htmlspecialchars( $_REQUEST['template-name'] ) : __( 'No Title', 'aq-page-builder' );
...
switch( $action ) {
	...
	case 'create' :
		...
		if( !is_wp_error( $new_id ) ) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . __( 'The ', 'aq-page-builder' ) . '<strong>' . $template_name . '</strong>' . __( ' page template has been successfully created', 'aq-page-builder' ) . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$messages[] = '<div id="message" class="error"><p>' . __( 'Sorry, the operation was unsuccessful for the following reason(s): ', 'aq-page-builder' ) . '</p>' . $errors . '</div>';
		}
	...
	case 'update' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __( 'The ', 'aq-page-builder' ) . '<strong>' . $template_name . '</strong>' . __( 'page template has been updated', 'aq-page-builder' ) . '</p></div>';
		break;
	...
	case 'delete' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __( 'The template has been successfully deleted', 'aq-page-builder' ) . '</p></div>';
		break;
}
	...
	<div id="page-builder-frame">
		...
		<div id="page-builder-column" class="metabox-holder <?php echo $disabled ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php _e( 'Available Blocks', 'aq-page-builder' ) ?></span><span id="removing-block"><?php _e( 'Deleting', 'aq-page-builder' ) ?></span></h3>
				<div class="inside">
					<ul id="blocks-archive" class="cf">
						<?php $this->blocks_archive() ?>
					</ul>
					<p><?php _e( 'Need help? Use the Help tab in the upper right of your screen.', 'aq-page-builder' ) ?></p>
				</div>
			</div>
		</div>
		...
		<div id="page-builder-fixed">
				...
				<div class="aqpb-tabs-nav">
					...
					<div class="aqpb-tabs-wrapper">
						<div class="aqpb-tabs">
							...
							<span class="aqpb-tab aqpb-tab-add aqpb-tab-active"><abbr title="<?php _e( 'Add Template', 'aq-page-builder' ); ?>">+</abbr></span>
							<?php } else { ?>
							...
							?>">
								<abbr title="<?php _e( 'Add Template', 'aq-page-builder' ); ?>">+</abbr>
							</a>
							<?php } ?>
					...
					<form id="update-page-template" action="<?php echo $this->args['page_url'] ?>" method="post" enctype="multipart/form-data">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										...
										<label class="open-label" for="template-name">
											<span><?php _e( 'Template Name', 'aq-page-builder' ); ?></span>
											<input name="template-name" id="template-name" type="text" class="template-name regular-text" title="<?php _e( 'Enter template name here', 'aq-page-builder' ); ?>" placeholder="<?php _e( 'Enter template name here', 'aq-page-builder' ); ?>" value="<?php echo is_object( $selected_template_object ) ? $selected_template_object->post_title : ''; ?>">
										</label>
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'aq-page-builder' ) : __( 'Save Template', 'aq-page-builder' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
										...
										<div class="delete-action">
											...
											echo '<a class="submitdelete deletion template-delete" href="' . esc_url( add_query_arg(
												...
												admin_url( 'themes.php' )
											)) . '">' . __( 'Delete Template', 'aq-page-builder' ) . '</a>';
											?>
						...
						<div id="aqpb-body">
							...
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if( $selected_template_id === 0 ) {
									...
									echo __( 'To create a custom page template, give it a name above and click Create Template. Then choose blocks like text, widgets or tabs &amp; toggles from the left column to add to this template.', 'aq-page-builder' );
									echo '<br/><br/>';
									echo __( 'You can drag and drop the blocks to put them in the order you want. Click on the small arrow at the corner of each block to reveal additional configuration options. You can also resize each block by clicking on either side of the block (Note that some blocks are not resizable)', 'aq-page-builder' );
									echo '<br/><br/>';
									echo __( 'When you have finished building your custom page template, make sure you click the Save Template button.', 'aq-page-builder' );
									echo '</p>';
						...
						<div id="aqpb-footer">
							<div class="major-publishing-actions cf">
								<div class="publishing-action">
									<?php if( !empty($selected_template_id ) ) {
										submit_button( __( 'Save Template', 'aq-page-builder' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_footer' ) ); 
									} ?>
								</div><!-- END .publishing-action -->
							</div><!-- END .major-publishing-actions -->
						</div>
			...
			<p style="float:left"><small><?php echo __( 'Aqua Page Builder &copy; ', 'aq-page-builder' ) . date( 'Y' ) . __( ' by', 'aq-page-builder' ); ?> <a href="http://aquagraphite.com">Syamil MJ</a></small></p>
			<p style="float:right"><small><?php echo __( 'Version ', 'aq-page-builder' ) . AQPB_VERSION; ?></small></p>
